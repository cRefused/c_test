/*
 * Взял отсюда: https://otvet.mail.ru/answer/468274019
 * немного переделал под свои хотелки
*/
...
int main() {
  ...
  while(run)
  {
    // играем с заданой скоростью
    if (clock() - mt >= CPS * MT_FACTOR)
    {
...
*/
  ...
  while(i != 10)
  {
    ...
    mvprintw(line/3 + i, col/2 - 7, "%s", "**************"); i++;
    mvprintw(line/3 + i, col/2 - 7, "%s", "              "); i++;
    mvprintw(line/3 + i, col/2 - 9, "%s", "press 'q' to quit"); i++;
    mvprintw(line/3 + i, col/2 - 7, "%s", "              "); i++;
    i = 10;
  }
